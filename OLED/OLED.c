...
{
//	{OLED_Printf(128-6*6, 0, 6,"FPS %d", Timer_GetFPS());	}	//显示帧率;解除注释开启
//	
	uint8_t j;
	/*遍历每一页*/
	for (j = 0; j < 8; j ++)
...
void OLED_UpdateArea(uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height)
{
	...
	/*参数检查，保证指定区域不会超出屏幕范围*/
	if (X > 127) {return;}	//如果X大于127则打印无意义,故无需打印
	if (Y > 63) {return;}	//如果Y大于63则打印无意义,故无需打印
...
/**
  ...
  *           故调用显示函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_UpdateArea0(uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height)
{
	uint8_t j;
